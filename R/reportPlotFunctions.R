...
if(packageVersion('PAMscapes') < '0.5.6') {
...
effortToBins <- function(effort, bin='hour') {
  bind_rows(lapply(split(effort, effort$DriftName), function(x) {
    thisEffort <- timesToBins(c(x$start, x$end), bin=bin)
    thisEffort$DeploymentSite <- x$DeploymentSite[1]
    thisEffort$DriftName <- x$DriftName[1]
    thisEffort
  }))
...
plotRegionSeason <- function(x, PLOTFUN, ...) {
  ...
  reduce(plots, `/`) + plot_layout(nrow=length(plots), ncol=1)
}
