...
TemplDuration= 2.50     #(must be longer than ClickDuration or will generate error message)
if (ClickDuration > TemplDuration) stop("ERROR:  click duration is longer than template duration")
lowfilter= 20000     #waveform bandpass filter lower value in Hz
highfilter= 100000   #waveform bandpass filter higher value in Hz


# DB and Binaries for processing
db <- 'PamGuard64 2_00_16e ST4300HF.sqlite3'

bin <- '.\\20170822'

# Input path where original acoustic files are located 
Acoustic_files_folder <- 'C:\\Jay\\ACOUSTIC\\PAMGUARD\\Click Templates\\R programs\\ClickTemplatesFromPamGuard\\ExampleData\\'

# Input path to the XML file exported from PAMGuard 
xmlFile <- 'PamguardBinaryStoreUIDLog.xml'

# Set path where wav clips will be stored 
WavClipFolder <- './WavClips'
...
pps <- PAMpalSettings(db=db, binaries=bin, sr_hz='auto', filterfrom_khz=30, filterto_khz=100, winLen_sec=.0025, settings=xmlFile)

# Process PAMGuard data into the Acoustic Study Object 
data <- processPgDetections(pps, mode='db', id="BWC ClickTemplate")
