...
STSens<-read.csv(here('data','SoundTrapCalibration.csv'),header=TRUE)

DepIDDetails<-DeployDetails %>% 
  dplyr::filter(Data_ID==DepID) 

STSN<-DepIDDetails$Instrument_ID..serial.number. #SoundTrap Serial Number
STSens<-dplyr::filter(STSens,InstID==STSN) %>%
  select(Sensitivity)

DepArray<-DepIDDetails$Array_name

DepHP<- Array %>%
  dplyr::filter(Array_name ==DepArray)%>%
...
DepHP$CH1.Sens<-unique(DepSens$Hydrophone.Sensitivity.dB.re..1V.uPa[Ch1Ind])
DepHP$CH2.Sens<-unique(DepSens$Hydrophone.Sensitivity.dB.re..1V.uPa[Ch2Ind])

return(c(paste0('Channel 1: Sens=',DepHP$CH1.Sens,'dB',' Dist=',DepHP$CH1.Dist,'m'),
       paste0('Channel 2: Sens=',DepHP$CH2.Sens,'dB',' Dist=',DepHP$CH2.Dist,'m'),
       paste0('SoundTrap Sens= ',STSens,'dB'),
       paste0('Full System (Ch 1) = ',abs(STSens+DepHP$CH1.Sens),'dB')))
